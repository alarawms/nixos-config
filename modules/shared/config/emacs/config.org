...
#+BEGIN_SRC emacs-lisp
  (setq user-full-name "Mohammed Alarawi"
...
#+BEGIN_SRC emacs-lisp
  ...
  (when (system-is-mac)
    ...
    (setq exec-path (append '("/Users/alarawms/bin" "/opt/homebrew/bin" "/profile/bin" "/Users/alarawms/.npm-packages/bin" "/Users/alarawms/.nix-profile/bin" "/nix/var/nix/profiles/default/bin" "/usr/local/bin" "/usr/bin") exec-path)))
#+END_SRC
...
(add-to-list 'load-path "/nix/store/5cqpq7q9yjmsdgdmchl4p3vq9advb5kz-emacs-mu4e-1.10.8/share/emacs/site-lisp/elpa/mu4e-1.10.8/")
(require 'mu4e)
                                        ; for sending mails
(require 'smtpmail)
#+end_src
...
;; (after! mu4e
        ...
        ;; )
(setq mu4e-mu-binary "/Users/alarawms/.nix-profile/bin/mu")
...
;; or another choice of yours:dir
(setq mu4e-attachment-dir "~/Downloads/emails/")

;; rename files when moving - needed for mbsync:
(setq mu4e-change-filenames-when-moving t)

;; list of your email adresses:
(setq mu4e-personal-addresses '("m@malarawi.sa"
...
(setq mu4e-contexts
      `(,(make-mu4e-context
          :name "malarawi.sa"
          :enter-func
          (lambda () (mu4e-message "Enter m@malarawi.sa context"))
          :leave-func
          (lambda () (mu4e-message "Leave m@malarawi.sa context"))
          :match-func
          (lambda (msg)
            (when msg
              (mu4e-message-contact-field-matches msg
                                                  :to "m@malarawi.sa")))
          :vars '((user-mail-address . "m@malarawi.sa" )
                  (user-full-name . "Mohammed Alarawi")
                  (mu4e-drafts-folder . "/m@malarawi.sa/Drafts")
                  (mu4e-refile-folder . "/m@malarawi.sa/Archive")
                  (mu4e-sent-folder . "/m@malarawi.sa/Sent")
                  (mu4e-trash-folder . "/m@malarawi.sa/Trash")))

        ,(make-mu4e-context
          :name "kaust.edu.sa"
          :enter-func
          (lambda () (mu4e-message "Enter mohammed.alarawi@kaust.edu.sa context"))
          :leave-func
          (lambda () (mu4e-message "Leave mohammed.alarawi@kaust.edu.sa context"))
          :match-func
          (lambda (msg)
            (when msg
              (mu4e-message-contact-field-matches msg
                                                  :to "mohammed.alarawi@kaust.edu.sa")))
          :vars '((user-mail-address . "mohammed.alarawi@kaust.edu.sa")
                  (user-full-name . "Mohammed Alarawi")
                  (mu4e-drafts-folder . "/mohammed.alarawi@kaust.edu.sa/Drafts")
                  (mu4e-refile-folder . "/mohammed.alarawi@kaust.edu.sa/Archive")
                  (mu4e-sent-folder . "/mohammed.alarawi@kaust.edu.sa/Sent")
                  (mu4e-trash-folder . "/mohammed.alarawi@kaust.edu.sa/Trash")))
        ,(make-mu4e-context
          :name "thuwalbio.sa"
          :enter-func
          (lambda () (mu4e-message "Enter alarawms@thuwalbio.sa context"))
          :leave-func
          (lambda () (mu4e-message "Leave alarawms@thuwalbio.sa context"))
          :match-func
          (lambda (msg)
            (when msg
              (mu4e-message-contact-field-matches msg
                                                  :to "alarawms@thuwalbio.sa")))
          :vars '((user-mail-address . "alarawms@thuwalbio.sa")
                  (user-full-name . "Mohammed Alarawi")
                  (mu4e-drafts-folder . "/alarawms@thuwalbio.sa/Drafts")
                  (mu4e-refile-folder . "/alarawmm@thuwalbio.sa/Archive")
                  (mu4e-sent-folder . "/alarawms@thuwalbio.sa/Sent")
                  (mu4e-trash-folder . "/alarawms@thuwalbio.sa/Trash")))

        ,(make-mu4e-context
          :name "alarawms@gmx.com"
          :enter-func
          (lambda () (mu4e-message "Enter alarawms@gmx.com context"))
          :leave-func
          (lambda () (mu4e-message "Leave alarawms@gmx.com context"))
          :match-func
          (lambda (msg)
            (when msg
              (mu4e-message-contact-field-matches msg
                                                  :to "alarawms@gmx.com")))
          :vars '((user-mail-address . "alarawms@gmx.com")
                  (user-full-name . "Mohammed")
                  ;; check your ~/.maildir to see how the subdirectories are called
                  ;; e.g `ls ~/.maildir/alarawms@gmx.com'
                  (mu4e-drafts-folder . "/alarawms@gmx.com/Drafts")
                  (mu4e-refile-folder . "/alarawms@gmx.com/Archive")
                  (mu4e-sent-folder . "/alarawms@gmx.com/Sent")
                  (mu4e-trash-folder . "/alarawms@gmx.com/Trash")))))
      ...
      (save-excursion
        (let*
            ((from (save-restriction
             ...
             (account
              (cond
               ((string-match "m@malarawi.sa" from) "m@malarawi.sa")
               ((string-match "alarawms@thuwalbio.sa" from) "alarawms@thuwalbio.sa")
               ((string-match "mohammed.alarawi@kaust.edu.sa" from) "mohammed.alarawi@kaust.edu.sa")
               ((string-match "alarawms@gmx.com" from) "alarawms@gmx.com"))))
...
(defun timu-mu4e-view-save-attachment ()
  "Save one attachements in a selected directory using `read-directory-name'.
...
(defun timu-mu4e-view-save-attachments ()
  "Save All Attachements in a selected directory using `read-directory-name'.
This is a modified version of `mu4e-view-save-attachments'."
  (interactive)
  (cl-assert (and (eq major-mode 'mu4e-view-mode)
                  (derived-mode-p 'gnus-article-mode)))
...
(setq gnus-icalendar-org-capture-file "~/Org/event.org")
...
(setq bibtex-completion-bibliography '("~/.emacs.d/org/ref/library.bib"))

(setq citar-bibliography '("~/.emacs.d/org/ref/library.bib"))

;You may also set the respective note and library path variables as well for enhanced functionality:
(setq bibtex-completion-library-path '("~/.emacs.d/org/ref/library.bib")
       bibtex-completion-notes-path "~/.emacs.d/org/ref/notes/"
...
(setq citar-library-paths '("~/Zotero/storage/")
       citar-notes-paths '("~/.emacs.d/org/ref/notes/"))
#+End_src
